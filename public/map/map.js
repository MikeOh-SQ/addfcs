const nodes = [
  {
    ...
    summary: "간단 자기점검 6문항의 질문 본문과 scale을 정의합니다. 지표 기준 자체는 JSON이 아니라 코드에 있습니다.",
    bullets: [
  ...
  },
  {
    ...
    summary: "실행 기능 세부 점검 23문항의 질문 본문과 section 분류를 정의합니다. section 문자열이 subtype 집계에 직접 연결됩니다.",
    bullets: [
      "부주의 9문항, 반응 조절 9문항, 추가 확인 5문항 구조입니다.",
    ...
    schema: [
      ...
      ["questions[].section", "부주의 / 반응 조절 / 추가 확인"],
  ...
  },
  {
    ...
    short: "REACT 3종 정의 원본",
    summary: "REACT 3종인 신호 찾기, 멈춤 버튼, 균형 유지를 정의합니다. 수행 지표 계산 로직 자체는 바꾸지 않습니다.",
    bullets: [
    ...
    schema: [
      ...
      ["lines[].text", "화면에 노출되는 과제 안내 문구"]
    ],
    links: [
      "public/test1/test1.js, public/test2/test2.js, public/test3/test3.js가 각 과제 시작 안내와 규칙 문구를 읽습니다.",
      "REACT 완료 후 summarizeReactivity()와 computeAssessmentMetrics(record)가 패턴 해석과 수행 지표 집계에 연결됩니다."
    ]
  },
  {
    id: "local-self",
    ...
    summary: "ASRS 응답을 기반으로 프런트가 먼저 만드는 로컬 요약 스키마입니다. AI 응답이 오기 전 임시 패턴 해석이자 fallback입니다.",
    bullets: [
      "totalPositive는 부담 응답 문항 수입니다.",
      "attentionPositive는 앞 4문항, hyperPositive는 뒤 2문항 부담 응답 개수입니다.",
      "summary, attentionMessage, hyperMessage, guidance가 화면 fallback 문장입니다."
    ],
    schema: [
      ["totalPositive", "부담 응답 총 개수"],
      ["attentionPositive", "집중 유지 관련 부담 응답 개수"],
      ["hyperPositive", "반응 조절 관련 부담 응답 개수"],
      ["severity", "뚜렷함 / 살펴볼 만함"],
      ["summary", "상단 hero용 기본 요약"],
      ["attentionMessage", "집중 유지 설명"],
      ["hyperMessage", "반응 조절 설명"],
      ["guidance", "다음 단계 권고"]
    ],
    links: [
  ...
  },
  {
    ...
    summary: "세부 자기점검 응답을 프런트가 2단계로 요약합니다. 1차 집계(analyzeDsm) 후, 화면 표시용 quick analysis 스키마로 다시 가공합니다.",
    bullets: [
      "inattentionYes, hyperactivityYes, contextualYes를 먼저 집계합니다.",
      "subtype은 집중 유지 부담/반응 조절 부담/복합 실행 기능/큰 부담 낮음 패턴으로 분기합니다.",
      "quickAnalysis는 AI와 동일한 필드명(summary, subtype, inattention, hyperactivity, guidance)을 사용합니다."
    ],
    schema: [
      ["inattentionYes", "집중 유지 Yes 개수"],
      ["hyperactivityYes", "반응 조절 Yes 개수"],
  ...
  },
  {
    ...
    summary: "반응성 과제가 끝날 때마다 갱신되는 중간 요약입니다. Test1, Test2, Test3 수행 지표와 highlight를 사람이 읽기 쉬운 형태로 정리합니다.",
    bullets: [
      "inattention_signal.score는 Test1 signal_detection 수행 지표입니다.",
      "impulsivity_signal.score는 Test2 go_nogo 수행 지표입니다.",
      "activity_signal.score는 Test3 balance_hold 수행 지표입니다."
    ],
    schema: [
      ["summary", "가장 높은 영역을 한 줄로 설명"],
      ["highlights[]", "핵심 지표 카드 목록"],
      ["inattention_signal", "Test1 수행 지표"],
      ["impulsivity_signal", "Test2 수행 지표"],
      ["activity_signal", "Test3 수행 지표"]
    ],
    links: [
  ...
  },
  {
    ...
    summary: "이 사이트의 핵심 통합 스키마입니다. ASRS 응답, DSM 응답, 반응성 과제 결과를 하나의 metrics 객체로 합칩니다.",
    bullets: [
    ...
    schema: [
      ["asrsPositiveCount", "서버 기준 ASRS 부담 응답 수"],
  ...
  },
  {
    ...
    schema: [
      ...
      ["출력.attention", "집중 유지 해석"],
      ["출력.hyperactivity", "반응 조절 해석"],
      ["출력.guidance", "권고 문구"]
    ],
    promptText: `서비스: ADDFCS.COM 주의·집중·실행 기능 패턴 자기점검 모바일 웹 앱
역할: 간단 자기점검 결과를 짧고 공감적으로 해석하는 한국어 자기점검 보조 AI
중요: 의학적 진단 표현 금지, 자기점검 리포트와 추가 평가 권고 수준으로만 작성
간단 자기점검 기준:
- 1, 2, 3번 문항은 가끔(2) 이상이면 부담 응답
- 4, 5, 6번 문항은 자주(3) 이상이면 부담 응답
- 부담 응답 4개 이상이면 추가 평가를 적극 고려할 수 있는 수준
기록 JSON:
{record}
계산된 간단 자기점검 해석용 지표:
{analysis}
...
2. attention는 집중 유지 관련 강도를 2문장 이내로 설명
3. hyperactivity는 반응 조절 관련 강도를 2문장 이내로 설명
4. guidance는 자기점검 도구 한계와 다음 단계 권고를 2문장 이내로 설명
5. 한국어로만 작성`,
    links: [
...
4. hyperactivity는 반응 조절 영역 해석을 2문장 이내로 설명
5. guidance는 자기점검 도구 한계와 다음 단계 권고를 2문장 이내로 설명
6. 한국어로만 작성`,
    links: [
  ...
  },
  {
    ...
    short: "REACT AI 해석",
    summary: "반응성 과제 로컬 요약과 통합 metrics를 받아 summary, inattention, impulsivity, hyperactivity, guidance를 JSON으로 돌려주는 AI 엔드포인트입니다.",
    bullets: [
    ...
    promptText: `서비스: ADDFCS.COM 주의·집중·실행 기능 패턴 자기점검 모바일 웹 앱
역할: 반응성 과제 결과를 짧고 공감적으로 해석하는 한국어 자기점검 보조 AI
중요: 의학적 진단 표현 금지, 수행 기반 참고 결과와 추가 평가 권고 수준으로만 작성
표현 지침:
...
6. 한국어로만 작성`,
  ...
  },
  {
    ...
    promptText: `서비스: ADDFCS.COM 주의·집중·실행 기능 패턴 자기점검 모바일 웹 앱
중요: 의학적 진단 표현 금지, 자기점검 리포트와 권고 수준으로 작성
어조: 따뜻하고 전문적인 임상심리사처럼, 부드러운 경어체 한국어 사용
표현 지침: '목표 놓침(부주의)', '잘못된 반응(충동성)'처럼 쉬운 말로 설명
교차 분석 지침: 간단 자기점검과 게임 기반 객관 지표가 일치하는지 분명히 짚기
수치 사용 지침: JSON에 실제 존재하는 숫자만 인용, 없는 수치 추정 금지
간단 자기점검 해석 기준:
- 1, 2, 3번 문항은 가끔(2) 이상이면 일상 부담 응답
- 4, 5, 6번 문항은 자주(3) 이상이면 일상 부담 응답
- 부담 응답한 문항이 4개 이상이면 주의·집중 또는 실행 기능 부담이 비교적 크게 나타난 패턴
세부 자기점검 해석 기준:
- 부주의 6개 이상 Yes, 반응 조절 6개 미만 Yes면 집중 유지 부담 패턴
- 반응 조절 6개 이상 Yes, 부주의 6개 미만 Yes면 반응 조절 부담 패턴
...
additionalSuggestion은 꼭 필요할 때만 한 문장으로 제안`,
  ...
  },
  {
    ...
    bullets: [
      ...
      "record.dtx.scores와 totalScore가 숲 지표와 stage 배경을 결정합니다.",
      "plan.json은 플랜 튜토리얼 대사만 바꾸고, 지표 규칙은 코드가 담당합니다."
    ],
    schema: [
      ["plan.suggestions", "플랜 목표 기본 source"],
      ["planGame.goals", "수정된 목표 카드 상태"],
      ["dtx.scores.*", "미니게임/플랜 누적 지표"],
...
const LAYOUT_STORAGE_KEY = "addfcs-map-layout-v1";
